...
//Author: Christophe Delaere
  ...
  if (!gSystem->AccessPathName(fname)) {
    printf("accessing %s file from local machine\n",fname);
  ...
  // The datasets used here are the same as the default ones.
   TMultiLayerPerceptron *mlp =
       new TMultiLayerPerceptron("@nch,@msumf,@ptsumf,@acolin:5:3:type",
  		       "ptsumf",simu,"Entry$%2","(Entry$+1)%2");
   mlp->Train(ntrain, "text,graph,update=10");
   mlp->Export("test","python");
   TCanvas *mcanvas = new TCanvas("m_Canvas","Network shape");
